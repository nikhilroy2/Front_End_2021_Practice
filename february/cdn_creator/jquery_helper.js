...
// running line init
function initDots() {
    jQuery('.news h3, .card-slide .card-text').dotdotdot({
    ...
    jQuery('.drag-card-holder .card-slide .card-text').dotdotdot({
        callback: function(isTruncated) {},
        ellipsis: "\u2026 ",
        height: null,
        keep: null,
    ...
    jQuery('.card-info-holder .truncate-1').dotdotdot({
        callback: function(isTruncated) {},
        ellipsis: "\u2026 ",
        height: null,
        keep: null,
    ...
    jQuery('.card-info-holder .truncate-2').dotdotdot({
        callback: function(isTruncated) {},
        ellipsis: "\u2026 ",
        height: null,
        keep: null,
    ...
    jQuery('.card-info-holder .truncate-3').dotdotdot({
        callback: function(isTruncated) {},
        ellipsis: "\u2026 ",
        height: null,
        keep: null,
...
$('.all-topics').click(function() {
    if ($(this).is(':checked')) {
        $('.tags input[name="topics"]').attr('checked', true);
    } else {
        $('.tags input[name="topics"]').attr('checked', false);
    }
});


$('.all-portfolio').click(function() {
    if ($(this).is(':checked')) {
        $('.tags input[name="portfolio"]').attr('checked', true);
    } else {
        $('.tags input[name="portfolio"]').attr('checked', false);
    }
});

// initialize fixed blocks on scroll
function initStickyScrollBlock() {
    jQuery('[data-fixed]').stickyScrollBlock({
        setBoxHeight: true,
...
function initMobileNav() {
    jQuery('html').mobileNav({
        ...
        menuOpener: '.sidenav-opener,.side-navclose',
        hideOnClickOutside: true,
        menuDrop: '.navbar',

    });
    jQuery('html').mobileNav({
        menuActiveClass: 'aside-content-ac',
        menuOpener: '.btn-aside',
        hideOnClickOutside: true,
        menuDrop: '.aside-contents',
    });

    jQuery('.card-nbr').mobileNav({
        menuActiveClass: 'card-active',
        menuOpener: '.card-info-holder .zoom',
        hideOnClickOutside: true,
        menuDrop: '.card-info-holder',
    });
    jQuery('.snooze-title').mobileNav({
    ...
    /*
    jQuery('html').mobileNav({
        menuActiveClass: 'aside-portfolio',
        menuOpener: '.btn-portfolio',
        hideOnClickOutside: true,
        menuDrop: '.aside-search',
    });*/
}

// open-close init
function initOpenClose() {
    // jQuery('.menu-bar ul li').openClose({
    //     activeClass: 'active',
    //     opener: '.ico-area,.icon-notification,.icon-th',
    //     slider: '> .dropdown-main',
    //     animSpeed: 400,
    //     effect: 'slide',
    //     hideOnClickOutside: true,
    // });
    jQuery('body').openClose({
        activeClass: 'active-mega',
        opener: '.btn-mega-menu',
        slider: '.mega-menu-in',
        animSpeed: 400,
        effect: 'slide',
        hideOnClickOutside: true,
...
function initSlickCarousel() {
    ...
    jQuery('.btns-slider').slick({
        slidesToScroll: 1,
        rows: 0,
        prevArrow: '<button class="slick-prev"><i class="icon-angle-left"></i></button>',
        nextArrow: '<button class="slick-next"><i class="icon-angle-right"></i></button>',
        variableWidth: true,
        autoplay: false, /*rveni12/11/2020 changed autoplay true to false */
        infinite: true,
        slidesToShow: 11,
    });
    jQuery('.cards-slider-full').slick({
        slidesToScroll: 1,
        rows: 0,
        slidesToShow: 6,
        prevArrow: '<button class="slick-prev"><i class="icon-angle-left"></i></button>',
    ...
    /* 30 January 2021 cards-slider-mid-top for 2 
    cards in row according to team discussion and requirements.
    */
    jQuery('.cards-slider-mid-top').slick({
        slidesToScroll: 1,
        rows: 0,
        slidesToShow: 2,
        prevArrow: '<button class="slick-prev" style="left: -5px"><i class="icon-angle-left"></i></button>',
        nextArrow: '<button class="slick-next" style="right: -5px"><i class="icon-angle-right"></i></button>',
        responsive: [ {
            breakpoint: 1000,
            settings: {
                slidesToShow: 1
    ...
    jQuery('.cards-slider-mid2').slick({
        slidesToScroll: 1,
        rows: 0,
        slidesToShow: 4,
        prevArrow: '<button class="slick-prev"><i class="icon-angle-left"></i></button>',
    ...
    jQuery('.top-stories .news-wrap').slick({
        slidesToScroll: 1,
        rows: 0,
        slidesToShow: 5,
        prevArrow: '<button class="slick-prev"><i class="icon-angle-left"></i></button>',
        ...
        attachEvents: function() {
            ...
            self.outsideClickHandler = function(e) {
                if (self.options.hideOnClickOutside) {
                    ...
                    // this is added nikhilroy
                    if (!header?.contains(e.target)) {
                        self.hideSlide();
                    }
                    // it has may be something wrong
...
$(window).on('load', function() {
    $(".card-animate-load").each(function() {
        $(this).addClass("load-from-bottom");
    });
});

$(window).on('scroll', function() {
    $(".card-animate-view").each(function() {
...
$(".draggable").on({
    dragstart: function(e) {
        ...
        $target = $(e.target);
        e.originalEvent.dataTransfer.setData("html", $target[0].outerHTML);
...
$(".drag-card-holder").on({
    ...
    dragover: function(e) {
        ...
        e.originalEvent.dataTransfer.dropEffect = "move"; // "move", "link"
        return false;
    },
    ...
    drop: function(e) {
        ...
        $(e.target).removeClass("dropping");
        var $target = $(e.target);
        var dropHtml = e.originalEvent.dataTransfer.getData("html");
        $target.append(dropHtml);
        $("#saveOperation").prop('disabled', false);
        $("#cancelOperation").prop('disabled', false);
    }
});


// clonning js for CARDS
// if it is in one page one block then you may need same classes
$(function() {
    $(".item-clone-btn-1").on('click', function(e) {
        e.preventDefault();
        var ele = $(this).closest('.item-clone').clone(true);
        $(ele[0]).find('.blinking').attr('onmouseover', ''); // added nikhilroy
        $(ele[0]).find('.blinking').css('animation', 'none'); // added nikhilroy
        $(ele[0]).find('.number-red').attr('onmouseover', ''); // added nikhilroy
        // console.log(ele[0]);
        ele[0].children[1].remove() // for pin hover effect nikhilroy work
        $('.item-clone-parent-1 .drag-card-holder').append(ele);
        $(".item-clone-parent-1 .saveOperation").prop('disabled', false);
        $(".item-clone-parent-1 .cancelOperation").prop('disabled', false);

    });
    $(".item-clone-parent-1 .cancelOperation").on('click', function(e) {
        e.preventDefault();
        $('.item-clone-parent-1 .item-clone').remove();
        $(".item-clone-parent-1 .saveOperation").prop('disabled', true);
        $(".item-clone-parent-1 .cancelOperation").prop('disabled', true);
    });

    $(document).on('click', '.drag-card-holder .item-clone:only-child .close', function(e) {
        e.preventDefault();
        $(this).closest(".draggable, .item-clone").remove();
        $(".item-clone-parent-1 .saveOperation").prop('disabled', true);
        $(".item-clone-parent-1 .cancelOperation").prop('disabled', true);
    });
});
...
$(document).on('click', '.drag-card-holder .close', function(e) {
    e.preventDefault();
    $(this).closest(".draggable, .item-clone").remove();
});
...
$("#add-new-port-tag").click(function() {
    var newPortfolioVal = $("#portfolio-text").val();
    if (newPortfolioVal !== "") {
        $("#portfolio-tags").append('<li><label class="tag-checkbox"><input name="portfolio" type="checkbox"><span class="text">' + newPortfolioVal + '</span></label></li>');
        $("#portfolio-text").val("");
    }
});

$("#cancelOperation").click(function() {
    $(".drag-card-holder").html("");
    $("#saveOperation").prop('disabled', true);
    $("#cancelOperation").prop('disabled', true);
});

$(window).on('load', function() {
    $('#modal-signup, #modal-charity').modal('show');
});
...
(function(root, factory) {
    ...
    ! function(t) { "use strict";
        ...
        function e() { h = t(window), s = {}, o = {}, r = {}, t.each([s, o, r], function(t, e) { e.add = function(t) { t = t.split(" "); for (var i = 0, n = t.length; i < n; i++) e[t[i]] = e.ddd(t[i]) } }), s.ddd = function(t) { return "ddd-" + t }, s.add("truncated keep"), o.ddd = function(t) { return "ddd-" + t }, r.ddd = function(t) { return t + ".ddd" }, r.add("resize"), e = function() {} } var i = "dotdotdot",
            n = "3.2.2"; if (!(t[i] && t[i].version > n)) { t[i] = function(t, e) { this.$dot = t, this.api = ["getInstance", "truncate", "restore", "destroy", "watch", "unwatch"], this.opts = e; var n = this.$dot.data(i); return n && n.destroy(), this.init(), this.truncate(), this.opts.watch && this.watch(), this }, t[i].version = n, t[i].uniqueId = 0, t[i].defaults = { ellipsis: "… ", callback: function(t) {}, truncate: "word", tolerance: 0, keep: null, watch: "window", height: null }, t[i].prototype = { init: function() { this.watchTimeout = null, this.watchInterval = null, this.uniqueId = t[i].uniqueId++, this.originalStyle = this.$dot.attr("style") || "", this.originalContent = this._getOriginalContent(), "break-word" !== this.$dot.css("word-wrap") && this.$dot.css("word-wrap", "break-word"), "nowrap" === this.$dot.css("white-space") && this.$dot.css("white-space", "normal"), null === this.opts.height && (this.opts.height = this._getMaxHeight()), "string" == typeof this.opts.ellipsis && (this.opts.ellipsis = document.createTextNode(this.opts.ellipsis)) }, getInstance: function() { return this }, truncate: function() { this.$inner = this.$dot.wrapInner("<div />").children().css({ display: "block", height: "auto", width: "auto", border: "none", padding: 0, margin: 0 }), this.$inner.empty().append(this.originalContent.clone(!0)), this.maxHeight = this._getMaxHeight(); var t = !1; return this._fits() || (t = !0, this._truncateToNode(this.$inner[0])), this.$dot[t ? "addClass" : "removeClass"](s.truncated), this.$inner.replaceWith(this.$inner.contents()), this.$inner = null, this.opts.callback.call(this.$dot[0], t), t }, restore: function() { this.unwatch(), this.$dot.empty().append(this.originalContent).attr("style", this.originalStyle).removeClass(s.truncated) }, destroy: function() { this.restore(), this.$dot.data(i, null) }, watch: function() { var t = this;
                    this.unwatch(); var e = {}; "window" == this.opts.watch ? h.on(r.resize + t.uniqueId, function(i) { t.watchTimeout && clearTimeout(t.watchTimeout), t.watchTimeout = setTimeout(function() { e = t._watchSizes(e, h, "width", "height") }, 100) }) : this.watchInterval = setInterval(function() { e = t._watchSizes(e, t.$dot, "innerWidth", "innerHeight") }, 500) }, unwatch: function() { h.off(r.resize + this.uniqueId), this.watchInterval && clearInterval(this.watchInterval), this.watchTimeout && clearTimeout(this.watchTimeout) }, _api: function() { var e = this,
                        i = {}; return t.each(this.api, function(t) { var n = this;
                        i[n] = function() { var t = e[n].apply(e, arguments); return "undefined" == typeof t ? i : t } }), i }, _truncateToNode: function(e) { var i = [],
                        n = []; if (t(e).contents().each(function() { var e = t(this); if (!e.hasClass(s.keep)) { var o = document.createComment("");
                        ...
                        1 == a.nodeType ? this._truncateToNode(a) : this._truncateToWord(a) } }, _truncateToWord: function(t) { for (var e = t, i = this, n = this.__getTextContent(e), s = n.indexOf(" ") !== -1 ? " " : "　", o = n.split(s), r = "", h = o.length; h >= 0; h--)
                        if (r = o.slice(0, h).join(s), i.__setTextContent(e, i._addEllipsis(r)), i._fits()) { "letter" == i.opts.truncate && (i.__setTextContent(e, o.slice(0, h + 1).join(s)), i._truncateToLetter(e)); break } }, _truncateToLetter: function(t) { for (var e = this, i = this.__getTextContent(t), n = i.split(""), s = "", o = n.length; o >= 0 && (s = n.slice(0, o).join(""), !s.length || (e.__setTextContent(t, e._addEllipsis(s)), !e._fits())); o--); }, _fits: function() { return this.$inner.innerHeight() <= this.maxHeight + this.opts.tolerance }, _addEllipsis: function(e) { for (var i = [" ", "　", ",", ";", ".", "!", "?"]; t.inArray(e.slice(-1), i) > -1;) e = e.slice(0, -1); return e += this.__getTextContent(this.opts.ellipsis) }, _getOriginalContent: function() { var e = this; return this.$dot.find("script, style").addClass(s.keep), this.opts.keep && this.$dot.find(this.opts.keep).addClass(s.keep), this.$dot.find("*").not("." + s.keep).add(this.$dot).contents().each(function() { var i = this,
                            n = t(this); if (3 == i.nodeType) { if ("" == t.trim(e.__getTextContent(i))) { if (n.parent().is("table, thead, tbody, tfoot, tr, dl, ul, ol, video")) return void n.remove(); if (n.prev().is("div, p, table, td, td, dt, dd, li")) return void n.remove(); if (n.next().is("div, p, table, td, td, dt, dd, li")) return void n.remove(); if (!n.prev().length) return void n.remove(); if (!n.next().length) return void n.remove() } } else 8 == i.nodeType && n.remove() }), this.$dot.contents() }, _getMaxHeight: function() { if ("number" == typeof this.opts.height) return this.opts.height; for (var t = ["maxHeight", "height"], e = 0, i = 0; i < t.length; i++)
                        if (e = window.getComputedStyle(this.$dot[0])[t[i]], "px" == e.slice(-2)) { e = parseFloat(e); break }
                    var t = []; switch (this.$dot.css("boxSizing")) {
                        case "border-box":
                            t.push("borderTopWidth"), t.push("borderBottomWidth");
                        case "padding-box":
                            t.push("paddingTop"), t.push("paddingBottom") } for (var i = 0; i < t.length; i++) { var n = window.getComputedStyle(this.$dot[0])[t[i]]; "px" == n.slice(-2) && (e -= parseFloat(n)) } return Math.max(e, 0) }, _watchSizes: function(t, e, i, n) { if (this.$dot.is(":visible")) { var s = { width: e[i](), height: e[n]() }; return t.width == s.width && t.height == s.height || this.truncate(), s } return t }, __getTextContent: function(t) { for (var e = ["nodeValue", "textContent", "innerText"], i = 0; i < e.length; i++)
                        if ("string" == typeof t[e[i]]) return t[e[i]];
                    return "" }, __setTextContent: function(t, e) { for (var i = ["nodeValue", "textContent", "innerText"], n = 0; n < i.length; n++) t[i[n]] = e } }, t.fn[i] = function(n) { return e(), n = t.extend(!0, {}, t[i].defaults, n), this.each(function() { t(this).data(i, new t[i](t(this), n)._api()) }) }; var s, o, r, h } }(jQuery);
    return true;
}));
/*
     _ _      _       _
...
(function($) {
    ...
    Marquee.prototype = {
        ...
        destroy: function() {
            ...
            this.holder.removeClass(this.options.hoverClass).removeData('Marquee');
...
function hideImg(x) {
    document.getElementById("dropdowncontent" + x).style.display = "none";
}

function bigImg(x) {
    document.getElementById("dropdowncontent" + x).style.display = "block";
}
...
// working here nikhilroy
// for drag-drop-cards
// window.onmouseover = (e) => {
//         let header = document.querySelector('header');
//         if (!header?.contains(e.target)) {
//             let userDrop = document.querySelectorAll('.user-drop.dropdown-main');
//             userDrop.forEach(v => {
//                 $(document).ready(function() {
//                     $(v).slideUp()
//                     setTimeout(() => {
//                         $(v).addClass('js-slide-hidden')
//                     }, 500)
//                 })
//             });
//             $(document).ready(function() {
//                 $('.options-menu.dropdown-main').slideUp();
//                 setTimeout(() => {
//                     $('.options-menu.dropdown-main').addClass('js-slide-hidden')
//                 }, 500)
//             })
//         }
//     }
    ...
    // for hide-header 
// $('.sub-header .items li').css('transition', '.1s')
// $(document).ready(function() {
//    // $('.sub-header .items li a').css('color', '#fff');
//    // $('.sub-header .items li').addClass('p-1')
//     $('.sub-header .items li').click(function() {
//         $('.sub-header .items li').removeClass('outline_lg bg_primary2 ');
//         $(this).addClass('outline_lg bg_primary2');
//     })
// })

// for index_gen card onmouseover hide
$(document).ready(function() {
    $('.card-nbr.item-clone').click(() => {
        $('.card-active').mouseleave(() => {
            //	console.log('leave ')
            $('.card-nbr.item-clone').removeClass('card-active')
        })
        window.onscroll = () => {
            $('.card-nbr.item-clone').removeClass('card-active')
        }
        let cardHolder = document.querySelector('.card-active .card-info-holder');

        $('.card-active .card-info-holder').mouseover(() => {

            $('.card-active .card-info-holder').mouseleave((e) => {
                if (!cardHolder?.contains(e.target)) {
                    $('.card-nbr.item-clone').removeClass('card-active')
                }
            })
        })
...
// favorite_user.html && share.html onclick function

    function makeFavorite(val){
      if(val.classList.contains('favorite_done')){
        val.innerHTML = '&#9744;'
        val.classList.remove('favorite_done')
      } else{
        val.innerHTML = '&#9745;';
        val.classList.add('favorite_done')
...
// .... Modal Nesting use below function for button and remove data-dismiss attribute ....

function nestingModal(){
	console.log("I am nesting")
    $(".nestingModal").modal("hide")
}
// .... Modal Nesting use below function for button and remove data-dismiss attribute ....End



// ................Btn Approval function.........
function btnApproval(val){
    if(val.innerText === 'Accepted'){
        val.innerText = 'Waiting';
        val.classList.add('ripple_gray_3');
        val.classList.remove('ripple_green')

    } else {
        val.innerText = 'Accepted'
        val.classList.add('ripple_green');
        val.classList.remove('ripple_gray_3')

    }

}
// ................Btn Approval function.........End
